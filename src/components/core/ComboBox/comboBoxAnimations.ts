...
export function runComboBoxOpenAfterSqueeze({
  anchorRef,
  disabled,
  isGloss,
  groupSegment,
  ...
  onOpened,
  openingRef,
  preferStandardSqueeze = false,
}: RunComboBoxOpenAfterSqueezeOptions & {
  ...
  const squeeze =
    !preferStandardSqueeze && isGloss && groupSegment == null
...
export function useComboBoxOpeningRef() {
  return useRef(false);
}
